<template>
  ...
  <div id="app">
    ...
    <div class="info" v-show="!socket || !socket.connected">
      <el-form ref="form" :model="form" label-width="80px">
        ...
        <el-form-item label="角色">
          <el-select v-model="form.cha" prop="cha">
            <el-option label="法官" value="法官"></el-option>
            <el-option label="书记员" value="书记员"></el-option>
            <el-option label="原告" value="原告"></el-option>
            <el-option label="被告" value="被告"></el-option>
          </el-select>
        </el-form-item>
        <el-button type="primary" @click="joinRoom" size="small" :disabled="socket && socket.connected">加入庭审室
        </el-button>
        <el-button type="primary" @click="leaveRoom" size="small" :disabled="!(socket && socket.connected)">离开庭审室
        </el-button>
        <el-button type="primary" @click="startAction" size="small"
                   :disabled="!(socket && socket.connected && isReady)">我已准备好开始
        </el-button>
        <el-button type="primary" @click="openlaw" size="small" :disabled="socket && socket.connected">打开法律条文
        </el-button>
      </el-form>
    </div>
    <div class="scene">
      ...
      <!-- <div id="chat" v-show="socket && socket.connected"> -->
        <el-tabs v-model="activeTab" @tab-click="tabClick">
          ...
          <el-tab-pane label="庭审消息" name="chatTab">
            <div>
          ...
          <el-tab-pane :label="'法庭成员('+ onlineClients.length +'人)'" name="userTab">
...
export default {
  ...
  methods: {
    ...
    handleSendMesssage () {
      ...
      let data = {msg: this.sendingMsg, username: this.form.username ,cha: this.form.cha}
    ...
    joinRoom () {
      ...
      this.socket.on('join', (data) => {
        this.updateChatMessage({msg: data.username + '加入了庭审', type: 'sys'})
      })
      // 自己加入成功
